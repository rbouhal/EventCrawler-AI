    ...
    <div className={`sidebar ${selectedDate ? 'open' : ''}`}>
      ...
      {events.length > 0 ? (
        <div>
          ...
          <h2>Analysis and Reccomendations:</h2>
          { (rate > 0 && occupancy !== 'Select an option') ?
          (
          <div>
            <p>Increase price by: {calculatePriceIncrease(events, occupancy, rate).toFixed(2)}%</p>
            <p>Increased price: ${(rate + ((calculatePriceIncrease(events, occupancy, rate)/100) * rate)).toFixed(2)}</p>
          </div>
          )
          :
          (
    ...
    <div class="container">
      ...
      <div className="search-filters">
            <input
              type="text"
              name="Average rate"
              placeholder="Average rate of your Hotel"
